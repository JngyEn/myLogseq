		...
		- #+BEGIN_QUERY
		  {:title ["📅 已计划未开始 "]
		   :query [:find (pull ?b [*])
		           ...
		           (or-join [?b ?today]
		             ;; 1. 计划日期在未来
		             (and [?b :block/scheduled ?d1] [(> ?d1 ?today)])
		             ;; 2. 截止日期在未来
		             (and [?b :block/deadline ?d2] [(> ?d2 ?today)])
		             ;; 3. 引用了未来的日记页 (处理类似 [[2027-09-02]] 的情况)
		             (and [?b :block/refs ?ref]
		                  [?ref :block/journal-day ?d3]
		                  [(> ?d3 ?today)]))
		   ]
		   :inputs [:today]
		   :breadcrumb-show? true
